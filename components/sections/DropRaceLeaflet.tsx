...
export default function DropRaceLeaflet() {
  ...
  /** A pin click goes straight to the point: fresh voters get the vote
   *  modal, returning voters scroll to the product menu. Same behaviour for
   *  everyone — no cinematic, no motion-preference branch. */
  const handleMarkerClick = useCallback((loc: LocalLocation) => {
    ...
    (async () => {
      ...
      try {
        ...
        for (const loc of locations) {
          ...
          m.on("click", () => handleMarkerClick(loc));
        }
        cleanup = () => {
          map.remove();
        };
    ...
    >
      ...
      <DropRaceVoteModal
        ...
        onSubmitted={(target) => {
          ...
          setModalLocationId(null);
          // Vote → menu. The post-submit destination is always FlavourGrid.
          // Lenis no-ops in reduced-motion so the scroll is instant for
          // those users.
          scrollToFlavourGrid();
        }}
      />
